...
const Docente = () => {
    ...
    const fetchData = async () => {
        ...
        const data = await db.collection("Docentes").get();
    ...
    const handleDeleteOk = (e) => {
        ...
        firebase.firestore().collection("Docentes").doc(formEdit.getFieldValue('id')).delete().then(() => {
            notification.success({
                message: 'Success',
                description: 'Docente deletado com sucesso'
            })
            fetchData();
        }
        ).catch((error) => {
            notification.error({
                message: 'Error',
                description: 'Erro ao deletar o docente'
            })
        }).finally(() => {
            setOpenDelete(false);
    ...
    const handleAddOk = async (e) => {
        ...
        const password = 'docente2023'; // make sure to add a password field to your form
        try {
            ...
            await firebase.firestore().collection("Docentes").add({
            ...
            notification.success({
                message: 'Success',
                description: 'Docente registado com sucesso'
            });
            fetchData();
        } catch (error) {
            notification.error({
                message: 'Error',
                description: 'Erro ao registar o docente'
            });
        } finally {
            setOpenAdd(false);
            setConfirmLoading(false);
    ...
    const handleEditOk = (e) => {
        e.preventDefault();
        setConfirmLoading(true);
        firebase.firestore().collection("Docentes").doc(formEdit.getFieldValue('id')).update({
            name: formEdit.getFieldValue('name'),
            telephone: formEdit.getFieldValue('telephone'),
            mail: formEdit.getFieldValue('mail'),
            unit: formEdit.getFieldValue('unit'),
        }).then(() => {
            notification.success({
                message: 'Success',
                description: 'Informação do docente actualizada com sucesso'
            })
            fetchData();
        }).catch((error) => {
            notification.error({
                message: 'Error',
                description: 'Erro ao actualizar a informação do docente'
            })
        }).finally(() => {
            setOpenEdit(false);
            setConfirmLoading(false);
    ...
    const handleActivateDeactivate = (record) => () => {
        firebase.firestore().collection("Docentes").doc(record.id).update({
            status: record.status === 'active' ? 'inactive' : 'active'
        }).then(() => {
            notification.success({
                message: 'Success',
                description: record.status === 'active' ? 'Conta docente desactivado com sucesso' : 'Conta docente activado com sucesso'
            })
            fetchData();
        }).catch((error) => {
            notification.error({
                message: 'Error',
                description: 'Erro ao actualizar a conta docente'
            })
        });
    };
